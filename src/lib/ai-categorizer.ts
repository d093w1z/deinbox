...
export interface EmailCategory {
  category: 'important'|'newsletter'|'spam'|'transactional'|'social'|
...
class AICategorizerService {
  ...
  private readonly patterns = {
    newsletter: {
      keywords: [
        'newsletter', 'unsubscribe', 'digest', 'weekly', 'monthly', 'update',
        'news'
      ],
    ...
    promotional: {
      keywords:
          [
            'sale', 'discount', 'offer', 'deal', 'promotion', 'coupon', 'free',
            'limited time'
          ],
    ...
    social: {
      ...
      senderPatterns:
          ['facebook', 'twitter', 'linkedin', 'instagram', 'notifications@'],
    ...
    important: {
      keywords:
          ['urgent', 'important', 'asap', 'deadline', 'meeting', 'appointment'],
    ...
    patterns.subjectPatterns.forEach(pattern => {
      ...
      if (maxScore > 0) {
        Object.keys(scores).forEach(
            key => {scores[key] = scores[key] / maxScore})
      }
  ...
  generateCleanupSuggestions(emails: EmailMessage[]): CleanupSuggestion[] {
    ...
    const categorizedEmails =
        emails.map(email => ({email, category: this.categorizeEmail(email)}))
    ...
    sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6)
      ...
      if (oldPromotional.length > 0) {
        suggestions.push({
          ...
          impact: {
            ...
            spaceFreed:
                oldPromotional.reduce((sum, {email}) => sum + email.size, 0),
      ...
      newsletterSenders.forEach(
          ({sender, emails}) => {suggestions.push({
            action: 'unsubscribe',
      ...
      if (suspectedSpam.length > 0) {
        suggestions.push({
          ...
          impact: {
            ...
            spaceFreed:
                suspectedSpam.reduce((sum, {email}) => sum + email.size, 0),
            category: 'spam'
          }
        })
      }
  ...
  private getFrequentSenders(
      ...
      minCount: number): {sender: string; emails: EmailMessage[]}[] {
    const senderCounts:
        Record<string, EmailMessage[]> = {}

                                         categorizedEmails.forEach(
                                             ({email}) => {
                                               const sender =
                                                   this.extractEmail(email.from)
  ...
  private extractEmail(emailString: string):
      string{const match = emailString.match(/<(.+)>/)
  ...
  analyzeInteractionPatterns(emails: EmailMessage[]): {
    ...
    const lowEngagement = emails.filter(email => {
      ...
      return category.category === 'newsletter' || category.category === 'promotional'
    })
      ...
      const inactiveThreads =
          emails.filter(email => email.date < oneMonthAgo && email.isUnread)

      return {
        lowEngagement, neverOpened, frequentSenders, inactiveThreads
      }
    }

    // Smart filtering based on multiple criteria
    getSmartFilters(): {name: string
    description: string
    ...
    return [
      {
        ...
        query: (emails) => emails.filter(email => {
          const threeMonthsAgo = new Date()
    threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3)
    const category = this.categorizeEmail(email)
          return category.category === 'newsletter' && email.date < threeMonthsAgo
        }),
      ...
      },
      {
        ...
        estimatedImpact: 'Medium - Removes marketing emails, may include wanted offers'
      },
      {
      ...
      },
      {
        ...
        estimatedImpact: 'Medium - Likely irrelevant, but may contain important items'
      }
    ]
    }
}
