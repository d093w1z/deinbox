...
class CacheService {
  ...
  async cacheEmails(userId: string, emails: EmailMessage[], ttl = 1800):
      Promise<void> {
  ...
  async cacheEmailStats(
      userId: string, stats: Record<string, unknown>,
      ttl = 3600): Promise<void> {
    const key = `stats:${userId}`;
    await this.set(key, stats, ttl);
  }

  async getCachedEmailStats(userId: string):
  ...
  async cacheUnsubscribeInfo(
      userId: string, info: Record<string, unknown>[],
      ttl = 7200): Promise<void> {
    const key = `unsubscribe:${userId}`;
    await this.set(key, info, ttl);
  }

  async getCachedUnsubscribeInfo(userId: string):
      Promise<Record<string, unknown>[]|null> {
    const key = `unsubscribe:${userId}`;
    return this.get<Record<string, unknown>[]>(key);
  }
  ...
  async mset(keyValuePairs: Record<string, unknown>, ttl = 3600):
      Promise<void> {
    try {
      await this.connect();
